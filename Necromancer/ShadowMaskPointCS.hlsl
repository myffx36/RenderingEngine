...
TextureCube<float> texture_slot0 : register(t0);
	...
	uint group_index : SV_GroupIndex)
{
	...
	float3 l = 
		world_position - (light_position.xyz / light_position.w);
	float3 texcoord =
		normalize(l);
	...
	float shadow_map_distance = texture_slot0.Gather(default_sampler, 
		float3(texcoord.x, texcoord.y, texcoord.z)).x * 1.015;
	float actual_depth = length(l);

	//g_out_texture[screen_pos] = shadow_map_distance * 2000.0f;
